...
export default function EditPricingTablePage() {
  const router = useRouter()
  const params = useParams()
  const pricingTableId = params.id as string
  ...
  const fetchPricingTable = async () => {
    try {
      const token = localStorage.getItem('admin_token')
      const response = await fetch(`/api/v1/admin/pricing-tables/${pricingTableId}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      })
      ...
      if (result.success) {
        const table: PricingTable = result.data
        setFormData({
        ...
        // Parse pricing tiers from data
        if (table.data && table.data.tiers) {
          setPricingTiers(table.data.tiers)
        }
      } else {
        setError(result.error || 'Failed to fetch pricing table')
      }
    } catch (err: any) {
      setError(`Error: ${err.message || 'Unknown error'}`)
      console.error('Fetch pricing table error:', err)
    } finally {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(result.error || 'Failed to update pricing table')
      }
    } catch (err) {
      setError('An error occurred while updating the pricing table')
      console.error(err)
    } finally {
      setLoading(false)
    }
  ...
  if (error && !formData.departmentId) {
    return (
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="mb-6">
        <Link
          href={`/admin/departments/${formData.departmentId}`}
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <h1 className="text-2xl font-bold text-gray-900 mb-6">Edit Pricing Table</h1>
